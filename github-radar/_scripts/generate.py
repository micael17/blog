...
- ⚠️ 매우 중요: 출력은 아래 명시된 구조(제목 → TL;DR → 이게 뭔가 → ...)만 정확히 따르세요. "★ Insight" 같은 박스, "이 글에 대한 메모" 같은 메타 코멘트, 이 프롬프트의 지시를 다시 언급하는 prologue/epilogue는 일절 포함하지 마세요. 첫 줄은 반드시 "## "로 시작하는 제목이어야 합니다.
...
def assemble(article_body, repo, ctx):
    ...
    Robust to leading garbage (e.g., model-added Insight blocks, blank lines):
    finds the first line starting with '## ' as the title; everything before
    it is discarded.
    """
    ...
    if title_idx is None:
        # Fallback: model didn't produce a '##' title. Use first non-empty line.
        title_idx = next((i for i, line in enumerate(lines) if line.strip()), 0)
