...
### `_scripts/selector.py` (NEW)

후보 풀에서 발행할 레포(들)를 결정. (모듈명은 Python stdlib `select` 모듈과 충돌 회피를 위해 `selector`)
...
- 회귀 테스트: 고정된 candidates fixture에 대해 selector.py가 일관된 결과를 내는지
